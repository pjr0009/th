module PaypalService::API
  ...
  class Payments
    ...
    def request(create_payment, async: false)
      @lookup.with_active_account(create_payment[:merchant_id]) do |m_acc|
        if (async)
          proc_token = Worker.enqueue_payments_op(
            transaction_id: create_payment[:transaction_id],
            op_name: :do_request,
            op_input: [create_payment, m_acc])

          proc_status_response(proc_token)
        else
          do_request(create_payment, m_acc)
        end
      end
    end

    def do_request(create_payment, m_acc)
      ...
      create_payment_data = create_payment.merge(
        { payer_id: m_acc[:payer_id],
          invnum: Invnum.create(create_payment[:transaction_id], :payment)})
     
      request = MerchantData.create_chained_payment(create_payment_data)
      with_success(create_payment[:transaction_id],
        request,
        error_policy: {
    ...
    def request_cancel(token_id)
      ...
      unless (token.nil?)
        ...
        @logger.warn("Tried to cancel non-existent request: [token: #{token_id}]")
        Result::Error.new("Tried to cancel non-existent request: [token: #{token_id}]")
      end
    end

    ## POST /payments/:community_id/create?token=EC-7XU83376C70426719
    def create(token, async: false)
      @lookup.with_token(token) do |token|
        if (async)
          proc_token = Worker.enqueue_payments_op(
    ...
    def do_create(token)
      ...
      if response.success
        ...
        TokenStore.delete(response[:data][:transaction_id])
      end
    ...
    def get_payment(transaction_id)
      Maybe(PaymentStore.get(transaction_id))
        .map { |payment| Result::Success.new(APIDataTypes.create_payment(payment)) }
        .or_else { Result::Error.new("No matching payment for transaction_id: #{transaction_id}.")}
    end

    ## POST /payments/:community_id/:transaction_id/void
    def void(transaction_id, info, async: false)
      @lookup.with_payment(transaction_id, [[:pending, nil]]) do |payment, m_acc|
        if (async)
          proc_token = Worker.enqueue_payments_op(
            transaction_id: transaction_id,
            op_name: :do_void,
            op_input: [transaction_id, info, payment, m_acc])

          proc_status_response(proc_token)
        else
          do_void(transaction_id, info, payment, m_acc)
        end
      end
    end

    def do_void(transaction_id, info, payment, m_acc)
      void_payment(
        transaction_id,
        payment,
        :success,
    ...
    ## POST /payments/:community_id/create?token=EC-7XU83376C70426719
    def refund(refund_info, async: false)
      if (async)
        proc_token = Worker.enqueue_payments_op(
          transaction_id: refund_info[:transaction_id],
          op_name: :do_refund,
          op_input: [refund_info]
        )

        proc_status_response(proc_token)
      else
        do_refund(refund_info)
      end
    end

    def do_refund(refund_info)
      ...
      if existing_refund.nil?
        ...
        with_success(refund_info[:transaction_id],
          request,
          error_policy: {
            codes_to_retry: ["10001", "x-timeout", "x-servererror"],
            try_max: 3
    ...
    def create_payment(token)
      ...
      @lookup.with_merchant_account(token) do |m_acc|
        with_success(token[:transaction_id],
          ...
          order_details = create_order_details(ec_details)
                          .merge({transaction_id: token[:transaction_id]})
    ...
    def void_payment(transaction_id, payment, flow, m_acc, note = nil)
      with_success(transaction_id,
        ...
        with_success(transaction_id, MerchantData.create_get_transaction_details({
              receiver_username: m_acc[:payer_id],
              transaction_id: payment[:order_id] ? payment[:order_id] : payment[:authorization_id],
            })) do |payment_res|
    ...
    def handle_failed_authorization(payment, m_acc)
      -> (cid, txid, request, err_response) do
        if err_response[:error_code] == "10486"
          ...
          token = PaypalService::Store::Token.get_for_transaction(payment[:transaction_id])
          ...
          void_failed_payment(payment, m_acc).call(payment[:transaction_id], request, err_response)
        end
      end
    end
